...
test-coverage: ## Run tests with coverage report (local development only)
	pytest tests --cov=src/clear_bow --cov-report=term-missing
...
clean: ## Remove all build artifacts and temporary files
	...
	rm -rf dist/ build/ *.egg-info/ .pytest_cache/ venv/ .tox/ .coverage htmlcov/ states/
...
version-history-pypi: ## Show version history from PyPI and help with semantic versioning
	...
	@curl -s https://pypi.org/pypi/clear-bow/json | grep -o '"version":"[^"]*"' | cut -d'"' -f4 | sort -V
	@echo "\nGit tags:"
	@git tag -l | sort -V
...
version-history-testpypi: ## Show version history from TestPyPI and help with semantic versioning
	...
	@curl -s https://test.pypi.org/pypi/clear-bow/json | grep -o '"version":"[^"]*"' | cut -d'"' -f4 | sort -V
	@echo "\nGit tags:"
	@git tag -l | sort -V
	@echo "\nSemantic Versioning Guide:"
	@echo "  MAJOR.MINOR.PATCH[-PRERELEASE]"
